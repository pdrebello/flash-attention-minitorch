...
# Targets
all: combine layernorm softmax flash flash_causal flash2
...
flash_fw_causal: src/flash_attn_fw.cpp
	cp src/flash_attn_fw.cpp src/flash_attn_fw.cu
	$(NVCC) $(CUDAFLAGS) -DCAUSAL_BLOCKSPARSE --shared src/flash_attn_fw.cu -o minitorch/cuda_kernels/flash_attn_causal_fw.so

flash_bw_causal: src/flash_attn_bw.cpp
	cp src/flash_attn_bw.cpp src/flash_attn_bw.cu 
	$(NVCC) $(CUDAFLAGS) -DCAUSAL_BLOCKSPARSE --shared src/flash_attn_bw.cu -o minitorch/cuda_kernels/flash_attn_causal_bw.so

flash2_fw: src/flash_attn2_fw.cpp
	cp src/flash_attn2_fw.cpp src/flash_attn2_fw.cu
	$(NVCC) $(CUDAFLAGS) -DCAUSAL_BLOCKSPARSE --shared src/flash_attn2_fw.cu -o minitorch/cuda_kernels/flash_attn2_fw.so

flash2_bw: src/flash_attn2_bw.cpp
	cp src/flash_attn2_bw.cpp src/flash_attn2_bw.cu
	$(NVCC) $(CUDAFLAGS) -DCAUSAL_BLOCKSPARSE --shared src/flash_attn2_bw.cu -o minitorch/cuda_kernels/flash_attn2_bw.so
