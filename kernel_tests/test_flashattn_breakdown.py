...
def test_launch_flash_attn_time():
  ...
  def attention_minitorch():
    ...
    dic["qk"] = time.time() - start_time
    inter_time = time.time()
      
    mask_mt = create_causal_mask(batch_size, nhead, from_len, to_len, q_mt.backend)
    res += mask_mt
    dic["mask"] = time.time() - inter_time
    inter_time = time.time()
      
    res = minitorch.nn.softmax(res , dim=3)
    dic["softmax"] = time.time() - inter_time
    inter_time = time.time()
      
    res = (drop * res)
    dic["dropout"] = time.time() - inter_time
    inter_time = time.time()
      
    res = res  @ v_mt
    dic["qkv"] = time.time() - inter_time
    inter_time = time.time()
    ...
    res = torch.tensor(res.to_numpy().astype(datatype)).cuda()
    print(dic)
    return [res], [end_time - start_time, inter_time - start_time, end_time - inter_time]
  ...
  def flash_attention_minitorch(causal=False, flash2=False):
    ...
    inter_time = time.time()
    print({'fused': inter_time - start_time})
    #out_mt.backward(out_grad_mt)
    end_time = time.time()
    #dq = torch.tensor(q_mt.grad.to_numpy().astype(datatype)).cuda()
    #dk = torch.tensor(k_mt.grad.to_numpy().astype(datatype)).cuda()
    #dv = torch.tensor(v_mt.grad.to_numpy().astype(datatype)).cuda()
    out = torch.tensor(out_mt.to_numpy().astype(datatype)).cuda()
    return [ out ], [end_time - start_time, inter_time - start_time, end_time - inter_time]
