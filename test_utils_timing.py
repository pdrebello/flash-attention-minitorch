...
class TestDecorator(object):
    ...
    def test(self, custom, baseline, nrepeat, rtol, atol):
        ...
        def core(func):
            ...
            return res, np.mean(timing, axis=0) * 1000

        print("Run baseline...")
        baseline_res, baseline_time = core(baseline)
        print("Run customs...")
        for custom_ in custom:
            custom_res, custom_time = core(custom_)
            print("Compare the results of custom and baseline...")
            self.assert_allclose(custom_res, baseline_res, rtol, atol)
            print("Baseline: " + ", ".join(["{:.3f}".format(x) for x in baseline_time]))
            print("Custom: " + ", ".join(["{:.3f}".format(x) for x in custom_time]))
            #import pdb
            #pdb.set_trace()
            #print(
